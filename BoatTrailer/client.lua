...
Citizen.CreateThread(function()
    while true do
        ...
        if veh ~= nil then
            if GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "BLAZER" then -- After a few hours here at 4am GetDisplayNameFromVehicleModel() got it working well :P
                local belowFaxMachine = GetOffsetFromEntityInWorldCoords(veh, 0.0, 1.0, 5.0)
				...
				if GetDisplayNameFromVehicleModel(GetEntityModel(trailerLoc)) == "BISON" then -- Is there a trailer????
